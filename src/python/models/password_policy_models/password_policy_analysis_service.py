...
class PasswordPolicyAnalysisService(AnalysisServiceBase):
    def analyze(self):
        try:
    ...
    def _analyze_length(self, length):
        try:
            required_length = int(self.settings.get('min_length', 8))
            length = int(length)
        except ValueError as e:
    ...
    def _analyze_lockout_duration(self, duration):
        try:
            required_duration = int(self.settings.get('lockout_duration', 30))
    ...
    def _analyze_lockout_obs_win(self, obs_win):
        try:
            required_obs_win = int(self.settings.get('lockout_obs_win', 30))
            obs_win = int(obs_win)
        except ValueError as e:
            logging.error("Invalid data - Lockout observation window or setting is not a number [service]",
                          exc_info=True)
